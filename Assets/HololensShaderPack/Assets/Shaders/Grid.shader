Shader "Hololens Shader Pack/Grid"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 doGrid(fixed ls, fixed lcr, fixed4 lc, fixed3 pos, fixed3 tpn)
			{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				o = transition.x * doGrid(_LineSize, _LineSize * max(0.01, (1 -_LineCrispness)), transition.y * _LineColorLarge, i.scaled.xyz, i.tpn);
				if (_FadeEnabled)
				{
					o *= i.fade.x;
				}
